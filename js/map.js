...
const layerState={bacia:true,municipios:false,sedes:true,geologia:false,solos:false,hidrografia:false,potencialidades:false};
const cache=new Map();
...
async function setLayer(key,on){layerState[key]=on;try{if(on&&!exists(key)) await adders[key]();ids[key].forEach(id=>{if(map.getLayer(id))map.setLayoutProperty(id,'visibility',on?'visible':'none')});updateLegend()}catch(err){setLoading(false);console.error(err);alert('Não foi possível carregar esta camada.')}}
...
async function fillMunicipioSelect(gj){
  ...
  gj.features.slice().sort((a,b)=>a.properties.NM_MUN.localeCompare(b.properties.NM_MUN,'pt-BR')).forEach((f,i)=>{const o=document.createElement('option');o.value=f.properties.CD_MUN;o.textContent=f.properties.NM_MUN;sel.appendChild(o)});
}
